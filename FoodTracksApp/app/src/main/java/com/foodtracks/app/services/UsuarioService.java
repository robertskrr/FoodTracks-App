...
public class UsuarioService implements IUsuarioService {
    ...
    public UsuarioService(IUsuarioRepository usuarioRepository, IRegistroBorradoRepository registroBorradoRepository) {
    ...
    public Task<Usuario> getPerfil(String uid) {
        return usuarioRepository.getUsuarioById(uid)
                .continueWithTask(task -> {
                    DocumentSnapshot doc = task.getResult();
                    // Si el usuario existe lo devuelve, en caso contrario devuelve la excepción NOT FOUND
    ...
    public Task<Void> registrarUsuario(Usuario usuario) {
        if (!emailValido(usuario.getEmail())) {
            return Tasks.forException(new UsuarioValidationException(R.string.email_validation_error_message));
        }
    ...
    public Task<Void> eliminarCuenta(String uidUsuario, String motivo, String uidAdmin) {
        return usuarioRepository.getUsuarioById(uidUsuario)
                .continueWithTask(task -> {
                    DocumentSnapshot doc = task.getResult();
                    ...
                    RegistroBorradoUsuario registro = RegistroBorradoUsuario.builder()
                    ...
                    return registroBorradoRepository
                            ...
                            .continueWithTask(deleteTask -> usuarioRepository.deleteUsuario(uidUsuario));
                });
    }
    ...
    public Task<Void> actualizarPerfil(Usuario usuarioNuevo) {
        ...
        return usuarioRepository.getUsuarioById(usuarioNuevo.getUid())
                .continueWithTask(task -> {
                    Usuario usuarioActual = task.getResult().toObject(Usuario.class);

                    // Comprueba si se ha cambiado el username
                    if (usuarioActual != null && !usuarioActual.getUsername().equals(usuarioNuevo.getUsername())) {
                        ...
                        return esUsernameUnico(usuarioNuevo.getUsername())
                                .continueWithTask(isUniqueTask -> {
                                    if (isUniqueTask.getResult() != null && !isUniqueTask.getResult()) {
                                        return Tasks.forException(new UsuarioValidationException(R.string.username_validation_error_message));
                                    }
    ...
    public Task<List<Usuario>> buscarUsuarios(String query) {
        ...
        return usuarioRepository.searchUsuariosByUsername(cleanQuery)
                .continueWith(task -> {
