...
public class RegisterClienteActivity extends AppCompatActivity {
    ...
    public void registroCliente(View view) {
        ...
        if (nombreReg.isEmpty()
                ...
                || emailReg.isEmpty()
                || passwordReg.isEmpty()) {
            Toast.makeText(getApplicationContext(), "Complete todos los campos", Toast.LENGTH_SHORT)
                    .show();
            return;
        }
    ...
    private void registrarUsuarioCliente(
            String nombre, String userName, String userEmail, String userPassword) {
        mAuth.createUserWithEmailAndPassword(userEmail, userPassword)
                .addOnCompleteListener(
                        new OnCompleteListener<AuthResult>() {
                            @Override
                            public void onComplete(@NonNull Task<AuthResult> task) {
                                // Si se completó correctamente guardamos los datos del nuevo
                                // usuario
                                if (task.isSuccessful()) {
                                    ...
                                    Map<String, Object> map =
                                            new HashMap<>(); // <Nombre campo, valor>
                                    map.put("uid", uid);
                                    ...
                                    if (otraPreferencia.isChecked()) {
                                        String preferenciaAlternativa =
                                                especifiqueOtro.getText().toString().trim();
                                        // Si ha marcado la casilla pero no ha puesto nada se da por
                                        // hecho que no tiene otra
                                        if (preferenciaAlternativa.isEmpty()) {
                                            map.put("otraPreferencia", false);
                                        } else {
                                            map.put(
                                                    "otraPreferencia",
                                                    preferenciaAlternativa.toLowerCase());
                                        }
                                    } else {
                                        map.put("otraPreferencia", false);
                                    }
